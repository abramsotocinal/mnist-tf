...
class Idx2Np:
    def __init__(self, target='train', data_dir=data_dir):
        ...
        if target not in self.fname.keys():
            raise ValueError(
                'Please specify one of the following targets: {}'.format(
                    self.fname.keys()))
        self.img = open(os.path.join(self.data_dir, self.fname[target]), 'rb')
        self.lbl = open(os.path.join(
            self.data_dir, self.fname[target + '_label']), 'rb')
        self.target = target
    ...
    def _to_array(self):
        ...
        self.array = np.fromfile(
            self.f, dtype=np.dtype(
                np.uint8)).newbyteorder(">").reshape(
            self.total, self.nC, self.nR)
        self.f.close()

    def _to_array_label(self):
        self.label_array = np.fromfile(
            self.f, dtype=np.dtype(
                np.uint8)).newbyteorder('>')
        self.f.close()
